...
class MiRobotSupervisor:
    ...
    def SeleccionarAccion(self, estado):
        ...
        # Explore by choosing a random action
            ...
            # Exploit by choosing the two actions with the highest Q-values
            q_values = self.Q[estado[0], estado[1], :]
            top_actions = np.argsort(q_values)[-2:]  # Get indices of top 2 actions
            accion = np.random.choice(top_actions)  # Randomly select one of the top actions
    ...
    def recompensa(self):
        ...
        reward = cambio_distancia  * 10# Multiplicador para ajustar la escala
        ...
        if estado_actual == target_state:
            reward += 300  # Recompensa por alcanzar el objetivo
    ...
    def entrenar(self, episodios):
        ...
        for episodio in range(episodios):
            ...
            while not fin:
                ...
                # Seleccionar la siguiente acción para el siguiente estado
                siguiente_accion = self.SeleccionarAccion(siguiente_estado)
    
                # Actualizar la matriz `Q` usando Q-learning
                if not fin:
                    max_next_q_value = np.max(self.Q[siguiente_estado[0], siguiente_estado[1], :])
                    target_q_value = recompensa + self.gamma * max_next_q_value
                    self.Q[self.estado[0], self.estado[1], accion] += self.alpha * (target_q_value - self.Q[self.estado[0], self.estado[1], accion])
...
rewards = rob.entrenar(30)
print(rewards)
# Save the Q-table to a pickle file
with open('q_table.pkl', 'wb') as f:
    pickle.dump(rob.Q, f)

best_actions = np.zeros((rob.num_boxes_x, rob.num_boxes_y), dtype=object)

for i in range(rob.num_boxes_x):
    for j in range(rob.num_boxes_y):
        best_q_values = np.argsort(rob.Q[i, j, :])[-2:]  # Get indices of top 2 actions
        best_actions[i, j] = best_q_values.tolist()  # Convert indices to list
